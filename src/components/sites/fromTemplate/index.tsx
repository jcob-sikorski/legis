...
function FromTemplate() {
  ...
  mockData = mockData?.map((x: any, i: number) =>
    i < templateIds?.length ? { ...x, template_id: templateIds[i] } : x
  );
    ...
    if (currentUser?.id) {
      ...
      async function createSite() {
        ...
        const newSite = {
          ...
          description:
            "Created from the template set with id: " + template_set_id,
          site_url: "",
          domainConnected: 0,
          favicon_url: "",
          cname: "",
  ...
  return (
    ...
    >
      <Flex vertical className="my-5">
        ...
        {mockBought ? (
          <Title style={{ textAlign: "center", color: "#339" }} level={5}>
            Purchase succesful!
          </Title>
        ) : (
          <Button
            onClick={() => setMockBought(true)}
            type="primary"
