...
interface IFrameProps {
    ...
    const IFrame: React.FC<IFrameProps> = ({ children, style = {}, colors = [], cssString = "" }) => {
        ...
        if (contentRef && cssString) {
            ...
            try {
                ...
                if (styleElement) {
                    ...
                    const newStyleNodeIds = styleNodeIds?.map((nodeId: string) => {
                        
                        // DELETE PREVIOUS STYLE ELEMENT
                        // message.info()
                        try {
                            const previousStyleElement = innerDoc!.getElementById(nodeId);
                            previousStyleElement!.parentNode!.removeChild(previousStyleElement!);
                            return "";
                        } catch {
            ...
            } catch {message.error("Error loading css in IFrame."); }
        }
    }    
