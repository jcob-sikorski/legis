...
export default function LTeam1({data,  setContext} : any) {
    ...
    return <section id='our-team' 
        className="py-12 bg-black text-gray-100 sm:py-16 lg:py-20 xl:py-24" style={{fontFamily: 'var(--legis-font-main)'}}
        onClick={bgEditable ? () => setContext({ isSection: true, collection: 'lawyerDetails', seriableLabel: 'lawyer'}) : () => {}}
    ...
    <div className="px-4 mx-auto sm:px-6 lg:px-8 max-w-7xl">
        ...
        <div className="max-w-2xl mx-auto mt-12 space-y-12 sm:mt-16 lg:mt-20 ">
            ...
            {((data?.lawyerDetails ?? []).map((person: any, i: number) => {
                ...
                try { parentEditable = editableMap[i] } catch {}
                ...
                return <>
                    ...
                    <div
                        ...
                        onClick={() => parentEditable ? setContext({seriableId: person?.id, isGroup: true, cdnUUID: person?.cdnUUID, collection: 'lawyerDetails', seriableLabel: 'lawyer', index: i}) : () => {}} 
                        className="editable items-start sm:flex"
                    >
                        {/* {person?.id || "No id"} */}
                        <img className="editable object-cover rounded-full w-44 h-44 shrink-0" 
                            onMouseEnter={() => setEditable(false)}
                            onMouseLeave={() => setEditable(true)}
                            onClick={() => setContext({cdnUUID: person?.cdnUUID, collection: 'lawyerDetails', seriableLabel: 'lawyer', key: 'cdnUUID', type: 'image', ratio: 1, label: 'Profile Picture', index: i}) }  
