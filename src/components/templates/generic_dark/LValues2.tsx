    ...
    >
      <div className="px-4 mx-auto max-w-7xl sm:px-6 lg:px-8">
        <div className="max-w-xl mx-auto text-center">
          ...
          >
            <p className="editable text-xs font-semibold tracking-widest text-white uppercase">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 xl:grid-cols-3 gap-5 mt-12 sm:grid-cols-1 md:grid-cols-3 lg:mt-20 lg:gap-x-12">
          {(typeof data?.valuesList === "object" ? data?.valuesList : [])?.map(
            (value: any, i: number) => (
              <div className="transition-all duration-200 bg-gray-950 hover:shadow-xl">
                <div className="py-10 px-9">
                  <i
                    className={switchIcon(value?.name ?? "")}
                    style={{ fontSize: 64, color: "var(--legis-color-2)" }}
                  ></i>
                  <h3
                    onClick={() =>
                      setContext({
