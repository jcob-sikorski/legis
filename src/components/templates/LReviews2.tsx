...
export default function LReviews2({data, setContext}: any) {
   ...
   return <section className="py-12 sm:py-16 lg:py-20 bg-black" style={{fontFamily: 'var(--legis-font-main)'}}>
   <div className="px-6 mx-auto sm:px-8 lg:px-12 max-w-7xl">
       <div className="text-center">
       <h2 
        onClick={() => setContext({section_id: data?.section_id, key: 'title', type: 'text', label: 'Title', variantProperty: 'textAlign', placeholder: 'See what our clients are saying'})} 
       ...
       <div className="grid break-all grid-cols-1 gap-16 mt-12 lg:grid-cols-1 xl:grid-cols-2 md:mt-16 lg:gap-y-20">
            ...
            {((data?.reviews ?? []).map((obj: any, i: number) => 
                ...
                <div className="mt-6 sm:ml-6 sm:mt-0">
                    <blockquote>
                        <p 
                        onClick={() => setContext({ collection: 'reviews', seriableLabel: 'review', key: 'testimonial', label: 'Review', type: 'textarea', index: i})} 
                        className="editable font-sans text-xl font-normal text-opacity-50 text-white">
                            {obj?.testimonial || "[Client Review here...]"}
                        </p>
                    </blockquote>
                    <p className="editable mt-6 font-sans text-2xl font-normal text-white"
                    onClick={() => setContext({ collection: 'reviews', seriableLabel: 'review', key: 'clientName', label: 'Review', type: 'text', index: i})} 
                    >
