...
export function convertTemplateIdToLabel(template_id: string) {
  if (template_id.includes("Contact"))
    return PROFILES_METADATA_MAP.category.contact;
...
export function getGenerativeFieldsFromTemplateId(template_id: string) {
  ...
  return fields.filter(
    ({ generative }: JSONProfileField) => generative && generative > 0
  );
}
...
export function getPromptForGeneration(onboardingData: OnboardingData) {
  ...
  onboardingData.templateIds.map((template_id: string) => {
    ...
    generativeFields.map(
      ({ id }: JSONProfileField) => (promptFields[id] = "[FILL]")
    );
  ...
  const prompt = `
      Hey chatgpt. Given that my client is a ${
        onboardingData.lawyerField
      } lawyer, I want you to create labels for the wireframes of each section of a website.
...
export function getBodyTemplateFromTemplateSetId(templateIds: string[]) {
  return Array(7)
    .fill({})
    .map((_, i: number) =>
      i < templateIds.length ? { template_id: templateIds[i] } : {}
    );
}
...
export function getHeroImageURLFromBodyTemplate(body_template: object[]) {
  ...
  (body_template || []).map((section: any) => {
    if (section?.template_id?.toLowerCase()?.indexOf("hero") > -1)
      // "has hero in template_id name"
      image_url = section?.cdnUUID
        ? getUrl(section?.cdnUUID)
        : DEFAULT_IMAGE_URL;
  });
...
export function contains(string: string | undefined, regex: string) {
  // return true
